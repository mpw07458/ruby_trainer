  ...
  describe 'class methods' do
    ...
    describe '.most_revenue' do
      it 'returns the top x merchants ranked by revenue' do
        ...
        expect(Merchant.most_revenue.first.revenue).to eq(5_999_000)
        expect(Merchant.most_revenue.last.revenue).to eq(359_940)
      end
    end
  ...
  describe 'instance methods' do
    ...
    describe '#total revenue' do
      it 'returns the total revenue for that merchant across successful transactions' do
        expect(Merchant.first.total_revenue).to eq(5_999_000)
        expect(Merchant.last.total_revenue).to eq(359_940)
      end
    end

    describe '#top_revenue_by_date' do
      it 'returns the total revenue for that merchant for a specific invoice date x' do
        expect(Merchant.first.top_revenue_by_date('2018-02-03')).to eq(5_999_000)
